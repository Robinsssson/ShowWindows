...
class ImageProcess {
   public:
    ...
    QVector<double> LKOptricalFlow(cv::Mat &frame, cv::Mat &old_gray, std::vector<cv::Point2f>& p0);
        ...
        if (m_index == 1)
        {
            if(LKOptricalFlow_first_entry == false)
            {
                ...
                for(auto & i : LKOptricalFlow_color)
                    i = cv::Scalar(std::rand()%255, std::rand()%255, std::rand()%255);
                LKOptricalFlow_first_entry = true;
                return 0.;
            }
            auto val = LKOptricalFlow(mat,LKOptricalFlow_old_gray,LKOptricalFlow_p0);
            if (cvIsNaN(val[0]))
                return 0.;
            return val[0];
        }
        if (m_index == 2)
            return MatTranslate(mat);
        return 0.0;
    }
   private:
    ImageProcess() {
